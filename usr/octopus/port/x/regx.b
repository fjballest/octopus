...
# This file was /appl/acme/regx.b, changed for o/x.
implement Regx;
include "mods.m";
...
number(t : ref Tblk, r : Range, line : int, dir : int, size : int) : (int, Range)
{
	q0, q1 : int;

	{
		...
		None =>
			q0 = 0;
			q1 = 0;
			while(line>0 && q1<t.blen())
				if(t.getc(q1++) == '\n')
					if(--line > 0)
						q0 = q1;
			if(line==1 && t.getc(q1-1)!='\n')	# no newline at end - count it
				;
			else if(line > 0)
				raise "e";
		Fore =>
	...
	exception{
		* =>
			warnc <-= "address out of range";
			return (0, r);
	}
	return (0, r);
}

regexp(t : ref Tblk, lim : Range, r : Range, pat : string, dir : int) : (int, Range)
{
	found : int;
	sel : Rangeset;
	q : int;
	...
	if(pat == nil && rxnull()){
		warnc <-= "no previous regular expression";
		return (0, r);
	}
	...
	if(!found)
		warnc <-= "no match for regexp";
	return (found, sel[0]);
}

xgetc(a0 : ref Tblk, a1 : string, n : int) : int
{
	if(a0 == nil)
		return rgetc(a1, n);
	return tgetc(a0, n);
...
address(t : ref Tblk, lim : Range, ar : Range,
	a0 : ref Tblk, a1 : string, q0 : int, q1 : int,  eval : int) : (int, int, Range)
{
	dir, size : int;
	prevc, c, n : int;
	q : int;
	...
	while(q < q1){
		...
		';' =>
			...
			if(q>=q1 && t!=nil)	# rhs defaults to $
				...
				(eval, q, nr) = address(t, lim, ar, a0, a1, q, q1, eval);
				r.q1 = nr.q1;
			}
			return (eval, q, r);
		',' =>
			if(prevc == 0)	# lhs defaults to 0
				r.q0 = 0;
			if(q>=q1 && t!=nil && t!=nil)	# rhs defaults to $
				r.q1 = nc;
			else{
				(eval, q, nr) = address(t, lim, ar, a0, a1, q, q1, eval);
				r.q1 = nr.q1;
...
rxexecute(t : ref Tblk, r: string, startp : int, eof : int) : (int, Rangeset)
{
	flag : int;
	inst : ref Inst;
	tlp : int;
...
rxbexecute(t : ref Tblk, startp : int) : (int, Rangeset)
{
	flag : int;
	inst : ref Inst;
	tlp : int;
